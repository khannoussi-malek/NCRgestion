...
<div class='panel panel-default'>
  ...
  <div class='panel-body'>
    <form  method='post' action="{{CRUDBooster::mainpath('addcomite')}}" enctype="multipart/form-data">
      <input type="hidden" name="_token" value="{{csrf_token()}}">
      <div class='form-group'>
        <label>Client</label>
        <input type='text' name='client' id='client' required class='form-control' value='{{$client->client}} 'readonly/>
        <input type='text'  name='id_client' id='id_client' required class='hidden' value='{{$sites->id_client}} 'readonly/>
      </div>
      <div class='form-group'>
        <label>site</label>
        <input type='text' name='site' id='site' required class='form-control' value='{{$sites->site}}' readonly/>
        <input type='text' name='id_site' id='id_site' required class='hidden' value='{{$sites->id}}' readonly/>
      </div>
      ...
      <div class='form-group'>
        ...
        <input type='text' name='traitemente' id='traitemente' required class='form-control' '/>
      </div>
      <div class='form-group' >
        ...
        <tr>
          ...
          <td>
            <div class='form-group '>
              ...
              <input type='checkbox' id="mp"name='mp'checked  onclick="return false;" value='maintenance préventive'/>
            </div>
          </td>
          <td>
            <div class='form-group '>
    ...
    @foreach($storages as $s)
      ...
      if (i==id) {
        if (e.target.value > {{$s->quantite}}) {
          e.target.value=0;
          alert("vous n'avez pas ce quantite de cette pièce");
        }
      }
    @endforeach
    }
